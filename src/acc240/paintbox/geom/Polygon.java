...
import acc240.paintbox.details.ShapePanel;
...
public class Polygon extends FillableShape implements Poly {

    private int[] xs, ys;

    public Polygon(Point point, Color col) {
        this((int) point.getX(), (int) point.getY(), col);
    }

    public Polygon(Point point, Color col, float strokeWidth) {
        this(point, point, col, strokeWidth);
    }

    public Polygon(int x1, int y1, Color col) {
        this(x1, y1, x1, y1, col, 1.0f);
    }

    public Polygon(Point point1, Point point2, Color col) {
        this((int) point1.getX(), (int) point1.getY(), (int) point2.getX(), (int) point2.getY(), col, 1.0f);
    }

    public Polygon(Point point1, Point point2, Color col, float strokeWidth) {
        this((int) point1.getX(), (int) point1.getY(), (int) point1.getX(), (int) point1.getY(), col, strokeWidth);
    }

    public Polygon(int x1, int y1, int x2, int y2, Color col, float strokeWidth) {
        super("Polygon", strokeWidth);
    ...
    public Polygon(int[] xs, int[] ys, Color col) {
        this(xs, ys, col, 1.0f);
    }

    public Polygon(int[] xs, int[] ys, Color col, float strokeWidth) {
        super("Polygon", strokeWidth);
    ...
    @Override
    public Shape copy() {
        Polygon result = new Polygon(xs[0], ys[0], xs[1], ys[1], borderColor, strokeWidth);
    ...
    public void draw(Graphics p) {
        ...
        if (filled && filling != null) {
            ...
            page.fillPolygon(xs, ys, xs.length);
    ...
    public void fromText(String[] data) {
        ...
        if (data.length >= last + 9) {
            filled = true;
            r = (int) (data[last + 6].charAt(0));
            g = (int) (data[last + 7].charAt(0));
            b = (int) (data[last + 8].charAt(0));
            a = (int) (data[last + 9].charAt(0));

            if (data[last + 6].length() == 2 && data[last + 6].charAt(0) == 'X') {
                r = 128 + data[last + 6].charAt(1);
            }
            if (data[last + 7].length() == 2 && data[last + 7].charAt(0) == 'X') {
                g = 128 + data[last + 7].charAt(1);
            }
            if (data[last + 8].length() == 2 && data[last + 8].charAt(0) == 'X') {
                b = 128 + data[last + 8].charAt(1);
            }
            if (data[last + 9].length() == 2 && data[last + 9].charAt(0) == 'X') {
                a = 128 + data[last + 9].charAt(1);
            }

            filling = new Color(r, g, b, a);
            strokeWidth = Float.parseFloat(data[last + 10]);
        } else {
            filled = false;
            filling = null;
    ...
    public void fromTextOld(String[] data) {
        ...
        if (data.length >= last + 8) {
            filled = true;
            r = (int) (data[last + 5].charAt(0));
            g = (int) (data[last + 6].charAt(0));
            b = (int) (data[last + 7].charAt(0));
            a = (int) (data[last + 8].charAt(0));

            if (data[last + 5].length() == 2 && data[last + 5].charAt(0) == 'X') {
                r = 128 + data[last + 5].charAt(1);
            }
            if (data[last + 6].length() == 2 && data[last + 6].charAt(0) == 'X') {
                g = 128 + data[last + 6].charAt(1);
            }
            if (data[last + 7].length() == 2 && data[last + 7].charAt(0) == 'X') {
                b = 128 + data[last + 7].charAt(1);
            }
            if (data[last + 8].length() == 2 && data[last + 8].charAt(0) == 'X') {
                a = 128 + data[last + 8].charAt(1);
            }

            filling = new Color(r, g, b, a);
            strokeWidth = Float.parseFloat(data[last + 9]);
        } else {
            filled = false;
            filling = null;
